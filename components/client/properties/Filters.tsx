...
const categories: (PropertyCategory | "ALL")[] = [
  "ALL", "BUY", "RENT", "SELL", "PLOTS", "RESIDENTIAL", "COMMERCIAL",
];
...
const Filters = ({ value, onChange }: Props) => {
  const { states, cities, streets } = useMemo(() => {
    ...
    properties.forEach((p) => {
      ...
      if (!value.state || p.address.state === value.state) c.add(p.address.city);
      if ((!value.state || p.address.state === value.state) &&
          (!value.city || p.address.city === value.city)) st.add(p.address.street);
    });
    return {
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-6">
        <p className="font-display text-xs tracking-[0.3em] text-muted-foreground uppercase shrink-0 md:w-28">Category</p>
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-6">
        <p className="font-display text-xs tracking-[0.3em] text-muted-foreground uppercase shrink-0 md:w-28">Location</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 flex-1">
          <select
            value={value.state}
            onChange={(e) => onChange({ ...value, state: e.target.value, city: "", street: "" })}
          ...
          <select
            value={value.city}
            onChange={(e) => onChange({ ...value, city: e.target.value, street: "" })}
            className="h-11 px-3 bg-card border border-border font-body text-sm text-foreground hover:border-emerald focus:border-emerald outline-none cursor-pointer"
          >
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-6">
        <p className="font-display text-xs tracking-[0.3em] text-muted-foreground uppercase shrink-0 md:w-28">Price</p>
        <div className="flex-1">
          <div className="flex items-baseline justify-between mb-3">
            <span className="font-display text-base font-semibold">{formatQAR(value.priceMin)}</span>
            <span className="font-body text-xs text-muted-foreground uppercase tracking-[0.2em]">to</span>
            <span className="font-display text-base font-semibold">
              {value.priceMax >= PRICE_CEIL ? `${formatQAR(PRICE_CEIL)}+` : formatQAR(value.priceMax)}
            </span>
          </div>
          <div className="relative h-8">
            ...
            <div
              ...
              style={{ left: `${pct(value.priceMin)}%`, right: `${100 - pct(value.priceMax)}%` }}
            />
            <input
              type="range"
              min={PRICE_FLOOR}
              ...
              onChange={(e) => {
                const v = Math.min(Number(e.target.value), value.priceMax - 100000);
            ...
            <input
              ...
              onChange={(e) => {
                const v = Math.max(Number(e.target.value), value.priceMin + 100000);
