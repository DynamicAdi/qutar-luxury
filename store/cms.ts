...
const seedProperties: Property[] = [
  ...
  },
  {
    ...
    state: "Qatar",
    description:
      "Full-floor penthouse with 360° city skyline. Private rooftop terrace and infinity jacuzzi.",
    BedRooms: 4,
    Bathrooms: 5,
    Area: 5400,
    yearBuilt: 2022,
    furnishing: "Furnished",
    parking: 3,
    images: [p2.src, p1.src],
    amenities: ["Concierge", "Rooftop Pool", "Gym", "Spa"],
    features: ["Skyline View", "Private Lift", "Smart Home"],
    NearByLocations: [
      { name: "City Center Doha", type: "Mall", distanceKm: 0.8 },
    ],
    agent: {
      name: "Omar Faisal",
      phone: "+974 5500 3344",
  ...
  },
  {
    ...
    parking: 6,
    images: [p3.src, p1.src],
    amenities: ["Private Beach", "Boat Dock", "Helipad", "Pool", "Gym"],
    features: ["Beachfront", "Helipad", "Boat Access"],
    NearByLocations: [
      { name: "Lusail Marina", type: "Landmark", distanceKm: 0.4 },
    ],
    agent: {
      name: "Layla Al-Mansoori",
      phone: "+974 5500 1122",
  ...
  },
  {
    ...
    images: [p4.src],
    amenities: [],
    features: ["Corner Plot", "Road Access", "Utilities Ready"],
    NearByLocations: [],
    agent: {
      name: "Omar Faisal",
      phone: "+974 5500 3344",
  ...
  },
  {
    ...
    furnishing: "Furnished",
    parking: 2,
    images: [p5.src, p2.src],
    amenities: ["Pool", "Gym", "Concierge", "Kids Area"],
    features: ["Heritage Design", "Smart Home", "Private Garden"],
    NearByLocations: [
      { name: "Souq Waqif", type: "Landmark", distanceKm: 0.6 },
    ],
    agent: {
      name: "Sara Al-Thani",
      phone: "+974 5500 5566",
...
const seedLeads: Lead[] = [
  ...
  },
  {
    ...
    budget: "5M – 10M",
    status: "New",
    message: "Looking for penthouse with marina view.",
    createdAt: new Date().toISOString(),
  },
  {
    id: "l3",
    ...
    budget: "Rental",
    status: "Contacted",
    message: "Need rental for 12 months, family of 5.",
    createdAt: new Date().toISOString(),
  },
  {
    id: "l4",
    ...
    budget: "10M+",
    status: "New",
    message: "Want a private beach villa.",
    createdAt: new Date().toISOString(),
  },
];
...
const seedHome: HomeContent = {
  ...
  heroSubtitle:
    "Curated luxury residences, penthouses and estates by Qutar Luxury Properties.",
  heroCtaText: "Explore Collection",
  aboutTitle: "The Standard of Luxury in Qatar",
  aboutBody:
    "For over a decade, QLP has connected discerning clients with the most exceptional properties across Doha, Lusail and The Pearl.",
    ...
    (set) => ({
      ...
      linkProperty: (customerId, propertyId) =>
        set((s) => ({
            ...
            c.id === customerId
              ? {
                  ...
                  propertyIds: Array.from(
                    new Set([...c.propertyIds, propertyId]),
                  ),
                }
              : c,
          ),
          properties: s.properties.map((p) =>
            p.id === propertyId
              ? { ...p, status: "Sold" as PropertyStatus }
              : p,
          ),
        })),
      unlinkProperty: (customerId, propertyId) =>
        set((s) => {
          ...
          const stillOwned = customers.some((c) =>
            c.propertyIds.includes(propertyId),
          );
          return {
            customers,
            properties: s.properties.map((p) =>
              p.id === propertyId && !stillOwned
                ? { ...p, status: "Available" as PropertyStatus }
                : p,
            ),
          };
        }),
