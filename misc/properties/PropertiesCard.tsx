    ...
    >
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex gap-1.5">
          ...
          {p.status === "SOLD" && (
            <Badge className="bg-success text-success-foreground hover:bg-success rounded-md">
              Sold
            </Badge>
          )}
          {p.isHidden && (
            <Badge className="bg-foreground/80 text-background hover:bg-foreground/80 rounded-md backdrop-blur">
              Hidden
            </Badge>
          )}
        </div>
        <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <button
            ...
            title={p.isHidden ? "Unhide" : "Hide"}
            className="rounded-lg bg-card/95 backdrop-blur p-2 hover:bg-card shadow-card"
          >
            {Thread ? (
      ...
      <div className="p-4 flex-1 flex flex-col">
        <h3 className="font-grotesk font-semibold text-base leading-tight line-clamp-1">
          {p.title}
        </h3>
        ...
        <div className="mt-3 flex items-center gap-3 text-xs text-muted-foreground border-y border-border/60 py-2.5">
          {p.BedRooms > 0 && (
            <span className="flex items-center gap-1.5">
              <Bed className="h-3.5 w-3.5 text-primary" /> {p.BedRooms}
            </span>
          )}
          {p.Bathrooms > 0 && (
            <span className="flex items-center gap-1.5">
              <Bath className="h-3.5 w-3.5 text-primary" /> {p.Bathrooms}
            </span>
          )}
        ...
        <div className="mt-4 flex gap-2">
          {onEdit && (
            <Button
              onClick={onEdit}
              size="lg"
              className="flex-1 rounded-xl bg-primary text-primary-foreground shadow-gold hover:opacity-90"
            >
